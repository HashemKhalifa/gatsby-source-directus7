...
export const info = (msg, dontShow) =>
    dontShow ? {} : console.log('GSD7:'.blue, 'info'.cyan, msg);
export const warn = (msg, dontShow) =>
    dontShow ? {} : console.log('GSD7:'.blue, 'warning'.yellow, msg);
 ...
 */
export const createNodesFromFiles = (
    files,
    createNode,
    createRemoteFileNode,
    downloadFilesToLocal = true,
) =>
    Promise.all(
        files.map(async f => {
...
export const mapRelations = (entities, relations, files, removeWarning, removeInfo) => {
    ...
    Object.keys(junctionRelations).forEach(junction => {
        ...
        if (junctions.some(containsNullValue)) {
            junctions = junctions.filter(j => !containsNullValue(j));
            warn(
                `Only '${junctions[0].collection_one}' contains the relational field though.`,
                removeWarning,
            );
    ...
    collectionsWithFiles.forEach(c => {
        ...
        newEntities[c.collectionName] = newEntities[c.collectionName].map(e => {
            ...
            if (!fileObj || !fileObj.directus || !fileObj.directus.id) {
                ...
                warn(
                    `id '${e.directusId}' in collection '${c.collectionName}'. Please ensure`,
                    removeWarning,
                );
